...
string login(vector<string> cmd, int id) {
    ...
    if (!found) {
        ...
        fstream file;
        file.open("efs/server1.txt",ios::in);
        string old;
        file >> old;
        file.close();

        file.open("efs/server1.txt",ios::trunc);
        file.close();

        file.open("efs/server1.txt",ios::out);
        int num = stoi(old) + 1;
        file << to_string(num);
        file.close();
    }
            ...
            if (FD_ISSET(connfd , &readfds)) {
                ...
                //Echo back the message that came in 
                    ...
                    if (cmd[0] == "join") { // join
                        ...
                        fstream f1, f2, f3;
                        f1.open("efs/server1.txt",ios::in);
                        f2.open("efs/server2.txt",ios::in);
                        f3.open("efs/server3.txt",ios::in);
                        string n1, n2, n3;
                        f1 >> n1;
                        f2 >> n2;
                        f3 >> n3;
